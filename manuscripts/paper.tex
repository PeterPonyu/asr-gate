...
% Journal-agnostic article class. Not a venue submission kit.
...
confidence scores beat honest random deferral? We build \texttt{asr-gate}, which
...
% Index terms (not a venue keyword environment).
\vspace{0.3em}
...
\textbf{(C1) Certified selective triage.} \texttt{asr-gate} certifies, at
...
\begin{figure}[t]
  ...
  drives Learn-Then-Test \citep{angelopoulos2021ltt} construction of a
...
\texttt{asr-gate} consumes decode artifacts (an $N$-best list per utterance with
...
clipped to $[0,1]$ (insertions can exceed 1; the clip frequency is reported and is
$0$ on Aishell-1 test). We certify the \emph{macro}-CER (per-utterance mean) of
...
bounded-mean statistic $\mathbb{E}[(\ell-\alpha)\mathbf{1}\{s\ge\lambda\}]>0$, whose
...
permissive rejected threshold $\lambda^\star$. With probability at least $1-\delta$
over the calibration draw, the accepted set's macro-CER is at most $\alpha$. We fix
...
the excess-AURC is their difference. Significance is a matched-abstention permutation
...
\begin{figure}[t]
  ...
  additive noise. (a) Accepted-set macro-CER (error bars over 20 reseeds) stays
  under the $\alpha{=}2\%$ target with 0/20 violations at 25 and 15\,dB and 1/20 at
  5\,dB (within the $\delta{=}0.1$ budget). (b) Coverage cost of noise.}
  \label{fig:noise}
\end{figure}
...
\begin{table}[t]
  ...
  decoded all utterances. Values from the frozen
...
\begin{figure}[t]
  ...
  informative quantity; their speaker-blocked bootstrap 95\% CIs (all excluding zero) are
  in \cref{tab:holm}. The $^{*}$ marks the permutation rejection, which is saturated at
...
\begin{table}[t]
  ...
  ($0.01368$ and $0.01370$). Point estimates and CIs are read from the frozen
  audit result files.}
...
\begin{table}[t]
  ...
  Values from the main-run and expansion audit result files. The accepted-set (certified)
...
conflated with in-corpus Aishell). \Cref{tab:landscape} gives the certified frontier.
...
\texttt{asr-gate} and all frozen result files are released in a public GitHub
repository and archived under a Zenodo DOI for reproduction
(TODO-USER: insert the repository URL and Zenodo DOI before submission); every
figure and table regenerates from those files via the released number-registry and
figure-generation code, so all reported numbers are reproducible bit-for-bit
from the archived artifacts. The noise corpus (ESC-50, CC BY-NC family) is used as data
