# Score-family mapping — ASR landscape (CPU contrast)

| Field | Value |
|---|---|
| **Date** | 2026-08-10 |
| **Owner** | asr-gate (`omission_score_contrast_2026-08-10/`) |
| **Evidence SSOT (read-only)** | this directory: `SUMMARY.md` and `results.json` |
| **Status** | metric contracts + provenance frozen 2026-08-10 |
| **Honesty** | Claim kinds are measurement and trend (M+T). Official-table reproduction (kind R) was **never** established. This mapping is not a completed experimental sequence and is not a passed landscape bar. |
...
| **Artifact regeneration** | 2026-08-10 regenerates artifacts with **dual-vacuity semantics** (`oracle_vacuous_frac`, `deploy_vacuous_frac`) |
...
Any post-result edit to honesty/stratum fields **must** record the three hashes/timestamps above from the **run that produced the cited numbers**. Hand-editing `SUMMARY.md` / `results.json` aggregates without a logged re-run or documented array re-aggregation (input sha256 + exact command) is forbidden.

**Jieba vendor pin:** `vendor/` (jieba 0.42.1, tracked for reproducibility).  
**Private cache policy:** `.jieba_cache/` is gitignored (local runtime only); `__pycache__/` likewise ignored.
...
| (b) | Public archive / Zenodo | Forthcoming; **no live DOI** — do not cite. (A previously printed identifier is unpublished and must not be used.) |
| (c) | Legacy `asr-fm-reliability-research` trees | Skipped — primary asr-gate landscape succeeded |

**Pinned corpus/backbone for this run:** all 9 landscape test cells above (frozen landscape decodes). No GPU re-decode.

---

## Score polarity (quoted)

From the coupled vs decoupled CRC-omission findings that this mapping implements (not an official-table reproduction):
...
| Analogue | ASR score | Certificate arm | Rationale |
|---|---|---|---|
...
- When the hyp asserts **no** entity tokens → score = **0** (bottom of scale) → CRC abstains first on silent drops (mirrors a coupled/`conf` arm).
- Helper reuse: same jieba / `pred_ent` machinery (entity POS set + `debyte`), adapted to a **continuous hyp-only** mass rather than a binary flag.
- **Not** a leftover certificate score from another protocol; **not** shopping s2–s5 as coupled.
...
- `l_entity`: LCS-aligned fraction of ref entity chars omitted
...
Vacuity fields: `oracle_vacuous_frac` and `deploy_vacuous_frac` per arm (dual semantics).

**Expected direction (coupled vs decoupled polarity):** coupled deploy-valid ≫ decoupled deploy-valid on **both-arms nonvacuous** primary stratum.
